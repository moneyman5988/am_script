...
echo "This is testing: All jobs completed.\n";
...
function insertJobLog($local, $jobId, $executionDate, $section, $startDateTime) {
	...
	$sql = "INSERT INTO seed_rotation_run_test (job_id, execution_date, section, status, started_datetime)
...
function updateJobLog($local, $runId, $status, $message = null) {
	...
	$sql = "UPDATE seed_rotation_run_test
...
function insertPlannedSeedMovement($local, $runId, $fromServer, array $seed, array $destinationDomain, array $destinationList) {
	...
	$sql = "INSERT INTO seed_rotation_log_test (
				run_id,
				seed_email,
				seed_id,
				from_server,
...
function canSeedRotateToDomain($local, $jobId, $section, $seedEmail, $server, $destinationDomain) {
	...
	$sqlPending = "SELECT l.log_id
			FROM seed_rotation_log_test l
			INNER JOIN seed_rotation_run_test r ON r.run_id = l.run_id
...
function completeMovement($local, $logId) {
	...
	$sql = "UPDATE seed_rotation_log_test
...
function updateNextRotationSchedule($local, $jobId, $currentSection) {
	...
	// run -> stop 3 day -> run
	$sql = "UPDATE seed_rotation_job_test
...
function checkDuplicateCron() {
	...
	exec("pgrep -f seed_rotation_job_fork.php", $pids);
...
function processJob(array $job) {
	...
	echo "[".date('H:i:s')."] [$pid] This is testing Start: $startDateTime\n";
	echo "[".date('H:i:s')."] PID $pid start\n";
	echo "[".date('H:i:s')."] [$pid] Processing on server: $server\n";
	...
	try {
		...
		echo "PID [$pid] Skipping server $server: " . $e->getMessage();
	...
	// echo $summary . "\n";
	$summary = "Planning completed. Planned {$planResult['planned']} movement(s).";
	...
	echo "[$pid] Next rotation section: ({$nextSchedule['next_section']})\n";
